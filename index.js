const kg = [
  ...
  { id: 'hooks', neighbors: ['链表', '闭包'], data: '1、顺序存到链表中，所以 hooks 不能放到循环和条件块中。2、hooks 靠闭包实现状态读写。所以也会遇到闭包的一些缺点，如 useEffect 里的 state 不是最新，必须将 state 放到依赖中，或者用 useRef。' },
  ...
  { id: 'middleware', neighbors: ['compose', 'redux-thunk'], data: 'middleware 是通过 compose 串联的，compose 就像是管道，action 作为参数在 compose 的管道里不断被加工' },
